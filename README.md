...
- **Serial absent** — IRQ handler skips serial status polling; the RTS flow control writes that `ReadBuffer` (and so `Chrin`) makes are suppressed; XModem `LOAD`/`SAVE` return an error
