...
| $A042 | `StReadSector` | Read CF sector |
| $A045 | `StWriteSector` | Write CF sector |
| $A048 | `StWaitReady` | Wait CF ready |
| $A04B | `SetIOMode` | Set IO_MODE |
| $A04E | `GetIOMode` | Get IO_MODE |
| $A051-$A0FF | Reserved | `jmp UnimplementedStub` |
...
3. Implement `RtcWriteTime` / `RtcWriteDate` — set DS1685 SET bit in `RTC_CTRL_B` before writing, clear after
